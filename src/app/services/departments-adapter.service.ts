...
import {
  CreateDepartmentsDocument,
  DeleteDepartmentsDocument,
  DeleteDepartmentsMutation,
  DepartmentPartFragment,
  DepartmentsDetailsDocument,
  DepartmentsDetailsGQL,
  DepartmentsDetailsQuery,
  Exact,
  UpdateDepartmentsDocument,
...
export class DepartmentsAdapterService extends ApolloClientService {
  ...
  departmentsQueryRef:
    | QueryRef<DepartmentsDetailsQuery, Exact<{ [key: string]: never }>>
    | undefined = undefined;
  editedDepartment: DepartmentPartFragment | null = null;

  constructor(
    apollo: Apollo,
    private dGQL: DepartmentsDetailsGQL,
  ...
  fetch(): Observable<DepartmentsDetailsQuery['departments']> {
    const data = super.fetchValues<DepartmentsDetailsQuery>(
      DepartmentsDetailsDocument,
      'departments',
    );
    ...
    const input: any = {
      ...
      manager: {
        connect: this.qlFilterService.connectWhere(
          'id',
          department.get('manager')?.value,
          true,
        ),
      },
    };

    const mutation = !departmentId
      ? CreateDepartmentsDocument
      : UpdateDepartmentsDocument;
    const variables = !departmentId
      ? { input }
      : { where: { id: departmentId }, update: input };
    if (!departmentId) {
      // Create a new person
      ...
      input.manager.disconnect = this.qlFilterService.connectWhere(
        'id_NOT_IN',
        '',
        true,
      );
  ...
  removeDepartment(
    id: string,
  ): Observable<MutationResult<DeleteDepartmentsMutation>> {
    return super._apollo.mutate<DeleteDepartmentsMutation>({
      mutation: DeleteDepartmentsDocument,
      variables: { where: { id: id } },
    });
  }

  managePersonsInDepartment(
    personsIds: string[],
    department: DepartmentPartFragment,
    exisitingIds: string[],
    remove: boolean = false,
  ): Observable<MutationResult<UpdateDepartmentsMutation>> {
    const input: any = {
      ...
      persons: {
        connect: this.qlFilterService.connectWhere(
          'id',
          remove
            ? exisitingIds.filter((x) => !personsIds.includes(x))
            : personsIds.concat(exisitingIds),
        ),
        disconnect: this.qlFilterService.connectWhere('id_NOT_IN', ''),
      },
    };
    const variables = { where: { id: department.id }, update: input };
    return super._apollo.mutate<UpdateDepartmentsMutation>({
      mutation: UpdateDepartmentsDocument,
      variables,
    });
  }
}
