...
import {
  ...
  CreateRatesDocument,
  DeleteExperiencesDocument,
  DeleteExperiencesMutation,
  DeleteRatesDocument,
  DeleteRatesMutation,
  EditExperiencesDocument,
  Exact,
  InputMaybe,
  PersonWhere,
  PersonWithAllTypeFragment,
  PersonsWithAllDocument,
  PersonsWithAllGQL,
  PersonsWithAllQuery,
...
export class PersonAdapterService {
  personsQueryRef:
    | QueryRef<
        PersonsWithAllQuery,
        Exact<{ where?: InputMaybe<PersonWhere> | undefined }>
      >
    | undefined = undefined;
  personQueryRef:
    | QueryRef<
  ...
  fetch(): Observable<PersonsWithAllQuery['people']> {
    return this.apollo
      .watchQuery<PersonsWithAllQuery>({ query: PersonsWithAllDocument })
      .valueChanges.pipe(
        map(
  ...
  setPersonQueryRef(personId: string): void {
    this.personQueryRef = this.pGQL.watch(
      { where: { id: personId } },
      {
        fetchPolicy: 'cache-and-network',
        errorPolicy: 'all',
  ...
  refetch(
    personId: string,
  ): Promise<ApolloQueryResult<PersonsWithAllQuery>> | undefined {
  ...
  submitPerson<T>(
    ...
    const input: any = {
      ...
      skills: {
        connect: this.qlFilterService.connectWhere(
          'id',
          personForm.get('skills')?.value,
        ),
      },
      projects: {
        connect: this.qlFilterService.connectWhere(
          'id',
          personForm.get('projects')?.value,
        ),
      },
      departments: {
        connect: this.qlFilterService.connectWhere(
          'id',
          personForm.get('departments')?.value,
        ),
      },
      roles: {
        connect: this.qlFilterService.connectWhere(
          'id',
          personForm.get('roles')?.value,
        ),
      },
    };
    ...
    const variables = !personId
      ? { input }
      : { where: { id: personId }, update: input };
    if (!personId) {
      // Create a new person
      ...
      input.projects.disconnect = this.qlFilterService.connectWhere(
        'id_NOT_IN',
        '',
      );
      input.departments.disconnect = this.qlFilterService.connectWhere(
        'id_NOT_IN',
        '',
      );
      input.roles.disconnect = this.qlFilterService.connectWhere(
        'id_NOT_IN',
        '',
      );
  ...
  submitPersonExperience<T>(
    ...
    const variables = isCreate
      ? { input }
      : { where: { id: experience.get('id')?.value }, update: input };
    return this.apollo.mutate({
      mutation: mutationDocument,
      variables,
    }) as Observable<MutationResult<T>>;
  }
  ...
  submitPersonRates<T>(
    ...
    const variables = isCreate
      ? { input }
      : { where: { id: rate.get('id')?.value }, update: input };
    return this.apollo.mutate({
      mutation: mutationDocument,
      variables,
    }) as Observable<MutationResult<T>>;
  ...
  removePersonsExperience(
    id: string,
  ): Observable<MutationResult<DeleteExperiencesMutation>> {
    return this.apollo.mutate<DeleteExperiencesMutation>({
      mutation: DeleteExperiencesDocument,
  ...
  removePersonsRate(
    id: string,
  ): Observable<MutationResult<DeleteRatesMutation>> {
    return this.apollo.mutate<DeleteRatesMutation>({
      mutation: DeleteRatesDocument,
      variables: { where: { id: id } },
    });
  }
}
