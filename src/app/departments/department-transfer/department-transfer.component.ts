...
export class DepartmentTransferComponent implements OnInit, OnDestroy {
  ...
  constructor(
    ...
    this.subscription.add(
      this.departmentsAdapterService.departmentsQueryRef?.valueChanges.subscribe(
        ({ data, errors, loading }) => {
          if (errors) {
          ...
          this.department = data.departments.filter(
    ...
    this.subscription.add(
      this.personAdapterService?.personsQueryRef?.valueChanges.subscribe(
        ({ data, loading, errors }) => {
          if (loading) {
            this.isLoading = loading;
          }
  ...
  setTransferList(people: PersonWithAllTypeFragment[] | null): void {
    ...
    people.map((person) => {
      if (!peopleList.find((p) => p.key === person.id)) {
        peopleList.push({
          ...
          direction: person.departments.find((d) => d.id === this.departmentId)
            ? 'right'
            : 'left',
        });
      }
    ...
    this.departmentsEmployeesIds = [
      ...peopleList.filter((p) => p.direction === 'right').map((e) => e.key),
    ];
  ...
  filterOption(inputValue: string, item: TransferItem): boolean {
    return (
      item.title.toLocaleLowerCase().indexOf(inputValue.toLocaleLowerCase()) >
      -1
    );
  }
  ...
  change(ret: TransferChangeReturn): void {
    ...
    if (ret.from === 'right' && ret.to === 'left') {
      this.departmentsAdapterService
        ...
        .subscribe(({ errors, loading }) => {
          ...
          this.notification.create(
            ...
            `Users ${users.map(
              (u) => u,
            )} were successfully removed from the department.`,
          );
        });
      this.departmentsEmployeesIds = this.departmentsEmployeesIds.filter(
        (x) => !ids.includes(x),
      );
    ...
    this.departmentsAdapterService
      ...
      .subscribe(({ errors, loading }) => {
        ...
        this.notification.create(
          'success',
          'Success',
          `Users ${users} were successfully added to the ${
