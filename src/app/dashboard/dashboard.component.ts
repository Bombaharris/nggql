...
export class DashboardComponent implements OnInit, OnDestroy {
    ...
    this.subscription.add(
      this.filterForm.valueChanges.subscribe((values) => {
        ...
        this.personAdapterService.personsQueryRef?.refetch(
          this.qlFilterService.getVariables(),
        );
      }),
    );
  }
  ...
  removePerson(person: PersonWithAllTypeFragment): void {
    ...
    this.personAdapterService
      .removePerson<DeletePersonsMutation>(person.id, DeletePersonsDocument)
      .subscribe(
        () => {
          this.personAdapterService.personsQueryRef?.refetch();
        },
  ...
  openForm(
    formType: 'person' | null,
    person?: PersonWithAllTypeFragment,
  ): void {
  ...
  closeForm(personForm?: FormGroup<PersonForm>): void {
    if (personForm) {
      ...
      if (this.editedPerson) {
        this.personAdapterService
          .submitPerson<UpdatePeopleGQL>(personForm, this.editedPerson.id)
          .subscribe(
            () => {
              this.notification.create(
                'success',
      ...
      this.personAdapterService
        .submitPerson<CreatePeopleGQL>(personForm)
        .subscribe(
          () => {
            this.notification.create(
              'success',
