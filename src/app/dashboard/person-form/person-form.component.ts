...
export class PersonFormComponent implements OnInit {
  ...
  ngOnInit(): void {
    if (this.person) {
      ...
      this.personForm
        .get('skills')
        ?.patchValue(this.person.skills.map((s: Skill) => s.id));
      this.personForm
        .get('projects')
        ?.patchValue(this.person.projects.map((p: Project) => p.id));
      this.personForm
        .get('departments')
        ?.patchValue(this.person.departments.map((d: Department) => d.id));
      this.personForm
        .get('roles')
        ?.patchValue(this.person.roles.map((r: Role) => r.id));
    }
  }
